  ...
  return (
    <>
      ...
      >
        ...
        <NavbarMenu
          className={`absolute top-0 desktop:w-[400px] max-h-[600px] tablet:w-[400px] bg-green desktop:mx-auto mobile:w-full flex flex-col`}      >

          <div className="flex gap-5 justify-between my-auto !mt-3 px-2 pb-5">
            <div className="flex flex-col my-auto !mt-24 gap-7">
              <Link
                href="/mapa"
                className="text-white text-sm p-2 font-extrabold font-karla hover:bg-brown block"
              >
                {t2("card1")}
              </Link>
              <Link
              ...
              >
                {Object.entries(languageNames).map(([locale, name]) => (
                  <DropdownItem
                    ...
                    className={`px-3 py-2 rounded-md hover:bg-primary hover:text-white cursor-pointer flex items-center gap-2 ${selectedLocale === locale ? "bg-primary  font-medium text-white" : ""
                      }`}
                  >
                    <div className="flex items-center gap-2">
                      {FlagIcons[locale]()}
